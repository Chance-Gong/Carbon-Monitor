...
> **This section applies only when you received this file via `read_file` during a PR review task.**
> It does NOT apply when this skill is activated via `use_skill` in interactive chat — in that case skip straight to `## Mission`.
> Do NOT read any files under `.bob/skills/carbon-builder/references/` for review tasks.

You have already satisfied the "read skill file first" requirement by reading this file. Now:

1. **Verify** the Carbon API claim using `code_search`, `docs_search`, or `get_charts` (whichever is appropriate for the finding type).
2. **Quote** the tool response directly in `mcpEvidence` — a direct excerpt from the `example_clean`, `props_used`, `chunk_text`, or equivalent field.
3. **Set** `verificationSource: "carbon-builder"` in the review JSON — **not** `"carbon-mcp"`. Reading this file before making MCP calls is the exact condition that distinguishes `"carbon-builder"` from `"carbon-mcp"`.
4. **Set** `carbonVerified: true`.

If the MCP tool call fails or returns no usable result, fall back to `verificationSource: "carbon-mcp"` and note what was attempted.
