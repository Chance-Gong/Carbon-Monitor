...
Mandatory:
- Use Carbon MCP tools as ground truth for ALL Carbon API, prop, token, icon, accessibility, or design-system claims.
...
Before making any Carbon-specific claim about components, imports, icons, tokens, props, styling, layout, accessibility, or design rationale, verify it with Carbon MCP tools. If Carbon MCP is unavailable, omit the Carbon-specific claim.
...
Reviewed by: <agent>
Carbon verification policy: Carbon-specific claims require Carbon MCP verification.
...
These are hard requirements for the implementation and prompt:

- Carbon-specific claims must set `carbonVerified: true`.
- `verificationSource` must be `carbon-mcp` for Carbon-specific claims.
...
- Carbon-specific review findings must be grounded in Carbon MCP tools.
